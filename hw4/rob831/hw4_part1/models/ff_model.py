...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        # Turn numpy arrays into PyTorch tensors if input is not already
        if not torch.is_tensor(obs):
            obs = ptu.from_numpy(obs.copy())
        if not torch.is_tensor(acs):
            acs = ptu.from_numpy(acs.copy())
        for key, value in data_statistics.items():
            if not torch.is_tensor(value):
                data_statistics[key] = ptu.from_numpy(value.copy())
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        if not torch.is_tensor(observations):
            observations = ptu.from_numpy(observations.copy())
        if not torch.is_tensor(actions):
            actions = ptu.from_numpy(actions.copy())
        if not torch.is_tensor(next_observations):
            next_observations = ptu.from_numpy(next_observations.copy())
        for key, value in data_statistics.items():
            if not torch.is_tensor(value):
                data_statistics[key] = ptu.from_numpy(value.copy())
